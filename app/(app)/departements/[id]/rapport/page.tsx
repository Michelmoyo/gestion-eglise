...
type Donnees = Awaited<ReturnType<typeof getDonneesRapport>>;
...
export default async function RapportPage({
  ...
  // Période sélectionnée : aucune par défaut. Tant que le responsable n'a
  // pas explicitement choisi une plage de dates, on n'affiche ni ne
  // pré-calcule aucune donnée (pas de mois courant implicite) — pour éviter
  // qu'un rapport parte avec une période choisie automatiquement plutôt que
  // volontairement.
  const { debut: debutParam, fin: finParam } = await searchParams;
  const aujourdHui = new Date().toISOString().split("T")[0];
  const dateValide = /^\d{4}-\d{2}-\d{2}$/;

  const periodeDebut = dateValide.test(debutParam ?? "") ? debutParam! : "";
  const periodeFin =
    dateValide.test(finParam ?? "") && (!periodeDebut || finParam! >= periodeDebut)
      ? finParam!
      : "";
  const periodeChoisie = !!periodeDebut && !!periodeFin;

  let rapportExistant: { id: string; date_soumission: string } | null = null;
  let periodeLabel = "";
  let nomEglise = "";
  let contactLigne2 = "";
  let reference = "";
  ...
  let roster: Donnees["roster"] = [];
  let statsByActivite: Donnees["statsByActivite"] = [];
  let presenceActiviteParOuvrier: Donnees["presenceActiviteParOuvrier"] = {};
  let nbActifs: Donnees["nbActifs"] = 0;
  let nouveauxOuvriers: Donnees["nouveauxOuvriers"] = [];
  let suspendusOuvriers: Donnees["suspendusOuvriers"] = [];
  let statsCultes: Donnees["statsCultes"] = [];
  let presenceCulteParOuvrier: Donnees["presenceCulteParOuvrier"] = {};
  let soldeDebut: Donnees["soldeDebut"] = 0;
  let soldeFin: Donnees["soldeFin"] = 0;
  let mouvementsPeriode: Donnees["mouvementsPeriode"] = [];
  let listesSuivi: Donnees["listesSuivi"] = [];
  let pointsSuivi: Donnees["pointsSuivi"] = [];
  let suiviInclus: Donnees["suiviInclus"] = [];
  ...
  if (periodeChoisie) {
    ...
    const { data } = await supabase
      ...
      .single();
    rapportExistant = data;

    // Données calculées pour le rapport pré-rempli — bornées exactement à
    // [periodeDebut, periodeFin] (bornes incluses, cf. lib/rapport.ts).
    ({
      roster,
      statsByActivite,
    ...
    periodeLabel = format.periode(periodeDebut, periodeFin);
    ...
    nomEglise = parametres?.nom_eglise || "Église";
    contactLigne2 = [parametres?.telephone, parametres?.email].filter(Boolean).join(" · ");
    reference = `RAP-${periodeDebut.replace(/-/g, "")}-${normaliserReference(dept.nom)}`;
  }
  ...
  return (
    <>
      ...
      <div className="p-4 space-y-4">
        ...
        {/* Sélection de la période -- plage de dates libre, vide par
            défaut : aucune donnée n'est affichée tant qu'elle n'est pas
        ...
        {!periodeChoisie ? (
          <p className="text-center text-sm text-muted-foreground py-10">
            Choisissez une période ci-dessus pour afficher les données du rapport.
          </p>
        ) : (
          <>
              ...
              <CardContent className="space-y-3">
                ...
                <div>
                  <p className="text-xs font-semibold text-green-700 mb-1">
                    Adhérents sur la période {nouveauxOuvriers.length ? `(${nouveauxOuvriers.length})` : ""}
                  </p>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm">
                  Activités sur la période ({statsByActivite.length})
                </CardTitle>
              </CardHeader>
              <CardContent className="divide-y divide-border">
                {!statsByActivite.length ? (
                  <p className="text-sm text-muted-foreground py-2">Aucune activité sur la période.</p>
                ) : (
            ...
            <Card>
              ...
              <CardContent className="divide-y divide-border">
                {!statsCultes.length ? (
                  <p className="text-sm text-muted-foreground py-2">Aucun culte sur la période.</p>
                ) : (
                  statsCultes.map((c) => (
                ...
                {peutVoirDetailCaisse ? (
                  <div className="pt-2 border-t border-border divide-y divide-border">
                    {!mouvementsPeriode.length ? (
                      <p className="text-sm text-muted-foreground py-2">Aucun mouvement sur la période.</p>
                    ) : (
